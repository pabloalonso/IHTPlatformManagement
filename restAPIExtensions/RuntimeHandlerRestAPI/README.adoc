...
= Runtime Handler REST API
REST API to manage Runtimes

The project **RuntimeHandlerRestAPI** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/RuntimeHandlerRestAPI.git
cd RuntimeHandlerRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `RuntimeHandlerRestAPI-1.0.0.zip`
...
Take the built artifact `RuntimeHandlerRestAPI-1.0.0.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
